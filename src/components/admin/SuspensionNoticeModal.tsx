...
import AppealFormModal from '../common/AppealFormModal';
...
export default function SuspensionModal({
  ...
  const canAppeal = type !== 'warning' && !!userId;
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden animate-[fadeIn_0.3s_ease-out]">
          ...
          <div className="px-6 py-6">
            ...
            {(type === '1 week suspension' || type === '30 days suspension') && suspendedUntil && (
              <div className="mt-4 bg-orange-50 border border-orange-100 rounded-lg p-4 text-center">
                ...
                {countdown && <p className="text-xs text-orange-500 mt-1">{countdown}</p>}
              </div>
            )}
          ...
          <div className="px-6 py-4 border-t border-gray-100 space-y-2">
            {canAppeal && !appealSubmitted && (
              <button
                onClick={() => setShowAppealForm(true)}
                className="w-full py-2.5 text-primary text-sm font-semibold rounded-lg border border-primary cursor-pointer hover:bg-green-50 transition-colors"
              >
                Submit an Appeal
              </button>
            )}
            {canAppeal && appealSubmitted && (
              <div className="w-full py-2.5 text-center text-sm text-green-600 font-medium bg-green-50 rounded-lg border border-green-100">
